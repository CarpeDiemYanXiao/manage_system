...
<script setup>
import {listReservation, payment, updateReservation, completeReservation} from "@/api/housekeeping/reservation.js";
...
const handleComplete = (reservation) => {
    ...
    ElMessageBox.confirm(
        ...
        .then(() => {
            completeReservation(reservationId).then(res => {
                getList()
                ElMessage({type: 'success', message: '服务已确认完成!',})
            }).catch(err => {
                ElMessage({type: 'error', message: err.msg || '操作失败',})
            })
        })
        .catch(() => {
        })
