...
import com.housekeeping.system.service.ISysPostService;
...
//    @PreAuthorize("@ss.hasPermi('system:user:query')")
    ...
    public AjaxResult recharge(@PathVariable Double money) {
        //获取当前登录用户的userId
        Long userId = getUserId();
        //根据用户ID查询充值前的余额
        Double balance = userService.selectUserById(userId).getBalance();
        if (money <= 0) {
            return error("请选择或输入正确的充值金额!");
        } else {
